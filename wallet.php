...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (password_verify($pin, $user['pin'])) {
        if ($type == 'deposit') {
            ...
            $pdo->prepare("INSERT INTO transactions (user_id, type, amount, status) VALUES (?, 'deposit', ?, 'completed')")->execute([$user_id, $amount]);
        } elseif ($type == 'withdrawal' && $wallet['balance'] >= $amount) {
            $pdo->prepare("UPDATE wallets SET balance = balance - ? WHERE user_id = ?")->execute([$amount, $user_id]);
            $pdo->prepare("INSERT INTO transactions (user_id, type, amount, status) VALUES (?, 'withdrawal', ?, 'completed')")->execute([$user_id, $amount]);
        } else {
            echo "<script>alert('Insufficient balance or invalid action');</script>";
        }
    } else {
...
<body>
    ...
    <div class="container">
        <h2>Balance: PKR <?php echo $wallet['balance']; ?></h2>
        <form method="POST">
